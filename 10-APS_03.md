...
Um avanço tecnológico que temos hoje é a biometria facial. Esse tipo de biometria tem sido usado em celulares, bancos e até mesmo para o controle de passaportes na imigração de aeroportos.

Neste roteiro, usaremos o Teorema de Bayes para determinar o valor econômico de um avanço nesta área.
...
Um [trabalho recente (de 2020)](https://ieeexplore.ieee.org/abstract/document/8970309) trouxe um sistema de reconhecimento facial para smartphones que mostrou uma taxa de falsos negativos igual à taxa de falsos positivos, ambas de 3.04%.
...
Neste momento, já sabemos calcular a probabilidade de um rosto ser legítimo dado o comportamento do sistema usando o Teorema de Bayes (se você não se lembra,  [veja este vídeo](https://youtu.be/bISWzyi0tDE), e depois [este outro](https://youtu.be/e2tog7ojm04)). Independente dessa probabilidade, podemos estimar a expectativa do custo de tomar uma decisão. Lembre-se que (revisar a [aula 6, exercício 3](06-amostras.ipynb)):

$$
E(X) = \sum_i P(x_i) c_i.
$$

Então, se o sistema aponta que o rosto é legítimo ($+$), temos um custo relacionado tanto ao rosto, de fato, ser legítimo ($L$), quando o custo relacionado ao rosto não ser legítimo ($\bar{L}$):

$$
E_{+}(X) = P(L | +) c_{L,+} + P(\bar{L} | +) c_{\bar{L},+},
$$

onde:

* $E_{+}(X)$ é a expectativa de custo caso o sistema aponte que o rosto é legítimo,
* $P(L | +)$ é a probabilidade do rosto ser legítimo dado que o sistema apontou que ele é, de fato, legítimo,
* $P(\bar{L} | + )$ é a probabilidade do rosto não ser legítimo dado que o sistema aponto que ele é legítimo (esta é a taxa de falsos positivos)
* $c_{L,+}$ é o custo operacional (por exemplo, em reais) para o caso em que o sistema apontou corretamente um rosto legítimo, e
* $c_{\bar{L},+}$ é o custo operacional para o caso de um falso negativo (por exemplo, o custo da consequência do acesso não-autorizado ao sistema).
...
Cada acesso não-autorizado ao prédio tem um custo de R$100,00 para a administração devido a questões como manutenção, seguro e indenizações pagas aos frequentadores. O acesso autorizado não tem custo adicional. Determine a expectativa de custo relacionado a essa operação, sabendo que há um total de 100 tentativas de acesso por dia no prédio.

### Parte 2

A administração do prédio decidiu que ele estava muito bem guardado, e então dispensou o profissional de segurança. A consequência disso é que passou a haver dez vezes mais tentativas de acesso por dia no prédio. A maioria desses acessos é ilegítimo, o que levou $P(L)$ a cair para $0.1$. Qual foi o aumento no custo da operação?
